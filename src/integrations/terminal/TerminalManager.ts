...
export class TerminalManager {
	...
	runCommand(terminalInfo: TerminalInfo, command: string): TerminalProcessResultPromise {
		...
		const extendedTerminal = terminalInfo.terminal as ExtendedTerminal
		if (extendedTerminal.shellIntegration) {
	...
	async getOrCreateTerminal(cwd: string): Promise<TerminalInfo> {
		...
		const availableTerminal = TerminalRegistry.getAllTerminals().find((t) => {
			...
			const extendedTerminal = t.terminal as ExtendedTerminal
			const terminalCwd = extendedTerminal.shellIntegration?.cwd // one of cline's commands could have changed the cwd of the terminal
			if (!terminalCwd) {
				return false
			}
